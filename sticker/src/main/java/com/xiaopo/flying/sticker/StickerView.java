...
public class StickerView extends FrameLayout {
  ...
  public StickerView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    try {
      ...
      borderPaint.setColor(a.getColor(R.styleable.StickerView_borderColor, Color.YELLOW));
  ...
  public void zoomAndRotateSticker(@Nullable Sticker sticker, @NonNull MotionEvent event) {
    if (sticker != null) {
      ...
      Log.e("mid point" ,String.format("%f %f", midPoint.x, midPoint.y));
      Log.e("event point", String.format("%f %f", event.getX(), event.getY()));
      Log.e("Old distance", String.valueOf(oldDistance));
      moveMatrix.set(downMatrix);
      moveMatrix.postScale(newDistance / oldDistance, newDistance / oldDistance, midPoint.x,
          midPoint.y);
      moveMatrix.postRotate(newRotation - oldRotation, midPoint.x, midPoint.y);
  ...
  public void zoomStickerBySeekbar(@Nullable Sticker sticker, float x)
  {
    float a, b;
    a = sticker.getCenterPoint().x;
    b = sticker.getCenterPoint().y;
    //moveMatrix.setScale(x, x,1 , 1);
    moveMatrix.set(downMatrix);
    moveMatrix.postScale(x,x, midPoint.x, midPoint.y);
    handlingSticker.setMatrix(moveMatrix);
    //if(sticker != null) handlingSticker.getMatrix().postScale(x, x, 0, 0);
...
public void mAddStickertoSpeceficPosition(Sticker sticker, float xPo, float yPo) {
  ...
  //sticker.getMatrix().preTranslate(xPo, yPo);
  //handlingSticker = sticker;
  handlingSticker = null;
